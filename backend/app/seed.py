...
def seed_db():
    ...
    try:
        ...
        import os
        # 2.5. Create Admin User conditionally based on ENV
        if os.getenv("CREATE_DEMO_ADMIN", "false").lower() == "true":
            admin_user = models.User(
                id="u-admin-001",
                name="System Administrator",
                email="admin@demo.healthtwin",
                hashed_password=auth.get_password_hash("admin1234"),
                role="ADMIN"
            )
            db.add(admin_user)
        else:
            print("Skipping Demo Admin user creation for production security.")
