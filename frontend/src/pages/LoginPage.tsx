...
export function LoginPage() {
  ...
  const handleDemoLogin = async (role: 'PATIENT' | 'DOCTOR' | 'ADMIN') => {
    ...
    setError('')
    const demoEmail = 
      role === 'PATIENT' ? 'alex.johnson@demo.healthtwin' : 
      role === 'DOCTOR' ? 'arun.mehta@demo.healthtwin' : 
      'admin@demo.healthtwin'
    const demoPass = 
      role === 'PATIENT' ? 'demo1234' : 
      role === 'DOCTOR' ? 'doctor1234' : 
      'admin1234'
    try {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="hidden w-1/2 flex-col justify-between bg-gradient-to-br from-navy-900 via-navy-800 to-indigo-900 p-12 lg:flex">
        ...
        <p className="text-xs text-navy-400">Sandbox Environment for System Evaluation</p>
      </div>
